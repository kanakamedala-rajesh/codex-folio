...
| --- | --- |
| Isolated identity profiles and installed-Codex launch | Implemented, including optional user-approved PATH setup from a direct binary run, guided first-profile setup from the plain command, existing-home adoption with valid sign-in reuse, separately consented history import offered after completed CLI and dashboard onboarding, resumable Pending profiles, a keyboard picker, and foreground Codex launch through the persistent companion; real-account platform qualification has documented exceptions |
| Read-only usage evidence and local analytics | Implemented; supported imported historical tokens include explicit coverage and Unassigned contributions, while provider-reported, derived, estimated, stale, and unavailable values remain distinguishable |
...
| Local dashboard and background operations | Authorized Overview, terminal-assisted Profiles onboarding with automatic completion status and lifecycle recovery, Shared Configuration Pack management, Sessions with explicitly consented supported-source import, Unassigned History, and reversible individual or bulk ownership correction, analytics, bounded operational Alerts, terminal-owned launches, Safe Continuation, bounded periodic usage collection, preview-before-download diagnostics and portable nonsecret configuration, separately consented update checks, and a schema-versioned optional telemetry client are implemented; plain startup now starts or reuses the full on-demand owner, initializes and remembers the qualified current-user protection choice (Windows DPAPI, macOS Keychain, Linux Secret Service, or the bundled Windows-user DPAPI bridge on WSL2), and gives inline locked/unavailable recovery guidance without browser opening or OS-login enrollment; explicit companion stop can defer for Managed Launches without ending Codex or removing OS-login enrollment; one-time guided background collection consent is remembered separately, and refusal preserves launches and on-demand refresh; explicitly trusted dashboard browsers renew short sessions across ordinary restarts over an installation-specific HTTPS origin after one-time browser certificate trust setup, and can be forgotten individually or revoked together; Linux can explicitly choose the repeated-interaction passphrase alternative, and an existing passphrase installation can migrate through one final private unlock with validated backup, atomic protected-state transition, destination reopen verification, and interruption recovery; the WSL bridge protects only as the interoperating Windows user and does not create a separate Linux-user boundary; imported profiles remain Pending until local Identity Home selection and Codex authentication; update checks remain unconfigured and telemetry enablement remains unavailable in production until their distinct project-controlled operational prerequisites are approved; explicitly enrolled services can use privacy-preserving native notifications with generic text by default and separate per-device detail consent; explicit per-user Task Scheduler, LaunchAgent, and systemd-user enrollment remains separate; native Tier 1 qualification remains limited as documented |
...
This summary reflects the implementation frontier on 2026-09-25. Follow the
